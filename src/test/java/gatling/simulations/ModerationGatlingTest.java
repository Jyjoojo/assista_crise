...
public class ModerationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all moderations").get("/api/moderations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new moderation")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    "\"motif\": \"SAMPLE_TEXT\"" +
                                    ", \"dateModeration\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_moderation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created moderation").get("${new_moderation_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created moderation").delete("${new_moderation_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Moderation entity").exec(scn);
