...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Gregoire.Duval96', email: 'Candide_Barbier94@gmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Reine_Mathieu');
      cy.get(`[data-cy="login"]`).should('have.value', 'Reine_Mathieu');

      cy.get(`[data-cy="firstName"]`).type('Angèle');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Angèle');

      cy.get(`[data-cy="lastName"]`).type('Leclercq');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Leclercq');

      cy.get(`[data-cy="email"]`).type('Agneflete_Barbier@hotmail.fr');
      cy.get(`[data-cy="email"]`).should('have.value', 'Agneflete_Barbier@hotmail.fr');
