...
import { sampleWithRequiredData } from '../moderation.test-samples';
...
describe('Moderation Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4073 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.moderations()[0]).toEqual(expect.objectContaining({ id: 4073 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4073 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.moderations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4073 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to moderationService', () => {
      const entity = { id: 4073 };
      vitest.spyOn(service, 'getModerationIdentifier');
      const id = comp.trackId(entity);
      expect(service.getModerationIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.moderationsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
