...
describe('Utilisateur Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./utilisateur-detail').then(m => m.UtilisateurDetail),
  ...
  describe('OnInit', () => {
    it('should load utilisateur on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', UtilisateurDetail);
