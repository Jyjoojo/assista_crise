...
import { sampleWithRequiredData } from '../salon.test-samples';
...
describe('Salon Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4107 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.salons()[0]).toEqual(expect.objectContaining({ id: 4107 }));
  });

  describe('trackId', () => {
    it('should forward to salonService', () => {
      const entity = { id: 4107 };
      vitest.spyOn(service, 'getSalonIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSalonIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.salonsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
